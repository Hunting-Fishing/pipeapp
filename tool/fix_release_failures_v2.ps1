...
if ((git branch --show-current).Trim() -ne 'main') {
  throw 'Run this release repair only from the main branch.'
}
...
if ($dirty.Count -gt 0) {
  Write-Host 'Working tree contains non-generated local changes:' -ForegroundColor Red
  $dirty | ForEach-Object { Write-Host $_ -ForegroundColor Red }
  throw 'Commit/stash the local source changes before running the release repair.'
}
...
$backupRoot = "${workspace}_release_fix_backup_$(Get-Date -Format yyyyMMdd_HHmmss)"
...
Write-Host '==> Applying release/test/accessibility repairs' -ForegroundColor Cyan
node .\tool\fix_release_failures_v2.mjs
Assert-NativeSuccess 'Release repair patcher'

Write-Host '==> Formatting changed Dart files' -ForegroundColor Cyan
dart format @targets
...
Write-Host '==> Analyzing lib + test' -ForegroundColor Cyan
...
Write-Host '==> Running the six previously failing test groups' -ForegroundColor Cyan
...
Write-Host '==> Removing generated plugin churn after Flutter tests' -ForegroundColor Cyan
foreach ($file in $generatedFiles) {
  git restore -- $file 2>$null
}
...
Write-Host '==> Staging only validated release repairs' -ForegroundColor Cyan
git add -- @targets
$staged = git diff --cached --name-only
if (-not $staged) {
  Write-Host 'No new release repairs needed; working tree is already current.' -ForegroundColor Green
  exit 0
}

Write-Host $staged

git commit -m 'Fix release test and accessibility blockers'
...
Write-Host ''
Write-Host 'Release repair validated and pushed to main.' -ForegroundColor Green
Write-Host 'Next: run tool\verify.ps1 again before Firebase production deployment.' -ForegroundColor Green
