...
Write-Step 'Synchronizing the seed-safe Directory repository repair bundle'
& git fetch origin design/formal-beautification-foundation
if ($LASTEXITCODE -ne 0) { throw 'STOP: Could not fetch seed-safe Directory repair controls.' }
...
if ($LASTEXITCODE -ne 0) { throw 'STOP: Could not synchronize seed-safe Directory repair bundle.' }
& git reset -q HEAD -- $supportFiles
if ($LASTEXITCODE -ne 0) { throw 'STOP: Could not unstage seed-safe Directory repair bundle.' }

Write-Step 'Parsing all seed-safe repair controls before production mutation'
$nodeControls = @(
  'tool/dispatch_directory_seed_safe_repository_transform.mjs',
  'tool/verify_dispatch_directory_seed_safe_repository_candidate.mjs',
  'tool/apply_dispatch_directory_seed_safe_repository_fix.mjs'
...
if ($parseErrors.Count -gt 0) {
  throw "STOP: Seed-safe Directory PowerShell parse failed: $($parseErrors[0].Message)"
}
Write-Host 'Seed-safe control parse preflight: PASS' -ForegroundColor Green

Write-Step 'Formatting synchronized tests/support only'
$testFiles = @(
...
if ($LASTEXITCODE -ne 0) { throw 'STOP: Could not normalize synchronized Directory tests.' }

Write-Step 'Proving existing Directory widget-test harness hygiene first'
...
$tempTestRelative = 'test/pipebuyer_directory_seed_safe_candidate_widget_test.dart'
$tempTestPath = Join-Path $repoRoot 'test\pipebuyer_directory_seed_safe_candidate_widget_test.dart'
$templatePath = Join-Path $repoRoot 'tool\templates\dispatch_directory_seed_safe_candidate_widget_test.dart.txt'
try {
  Write-Step 'Building exact local seed-safe candidate before production mutation'
  ...
  if ($LASTEXITCODE -ne 0) {
    throw 'STOP: Seed-safe Directory candidate transformation failed. Production source was not changed.'
  }
  if (-not (Test-Path -LiteralPath $candidatePath)) {
    throw 'STOP: Seed-safe Directory candidate was not created.'
  }

  & dart format $candidatePath
  if ($LASTEXITCODE -ne 0) {
    throw 'STOP: Formatter rejected seed-safe Directory candidate. Production source was not changed.'
  }
  & flutter analyze --fatal-infos --fatal-warnings $candidatePath
  if ($LASTEXITCODE -ne 0) {
    throw 'STOP: Analyzer rejected seed-safe Directory candidate. Production source was not changed.'
  }

  Copy-Item -LiteralPath $templatePath -Destination $tempTestPath -Force
  & dart format $tempTestPath
  if ($LASTEXITCODE -ne 0) {
    throw 'STOP: Could not format seed-safe candidate widget test.'
  }

  Write-Step 'Running candidate widget runtime proof WITHOUT Firebase initialization'
  & flutter test $tempTestPath --reporter expanded
  if ($LASTEXITCODE -ne 0) {
    throw 'STOP: Seed-safe candidate still fails seeded widget runtime proof. Production source was not changed.'
  }
  ...
  Write-Host 'Exact local candidate analyzer + seeded-widget runtime proof: PASS' -ForegroundColor Green
}
finally {
...
Write-Step 'Applying only the already-proven seed-safe repository lifecycle correction'
& node '.\tool\apply_dispatch_directory_seed_safe_repository_fix.mjs'
if ($LASTEXITCODE -ne 0) {
  throw 'STOP: Seed-safe Directory source repair failed after candidate runtime PASS.'
}
...
if ($LASTEXITCODE -ne 0) { throw 'STOP: Production Directory is not formatter-stable after seed-safe repair.' }
& flutter analyze --fatal-infos --fatal-warnings '.\lib\marketplace\marketplace_dispatch_directory.dart'
if ($LASTEXITCODE -ne 0) {
  throw 'STOP: Production Directory source does not compile after seed-safe repair. Do not rerun unrelated mutations.'
}
Write-Host 'Production Directory analyzer: PASS' -ForegroundColor Green

Write-Step 'Running seed-safe contract and Directory widget tests in isolation'
& flutter test `
  '.\test\dispatch_directory_seed_safe_repository_contract_test.dart' `
  '.\test\marketplace_dispatch_directory_test.dart' `
  --reporter expanded
if ($LASTEXITCODE -ne 0) {
  throw 'STOP: Seed-safe production Directory widget regression failed after candidate runtime PASS. Inspect first exception only; do not rerun older source repairs.'
}

Write-Step 'Running retained runtime and projection/query regressions'
& flutter test `
  '.\test\dispatch_directory_runtime_contract_hygiene_test.dart' `
  '.\test\dispatch_directory_filter_runtime_stability_contract_test.dart' `
  '.\test\marketplace_dispatch_directory_projection_query_test.dart' `
  --reporter expanded
if ($LASTEXITCODE -ne 0) {
  throw 'STOP: Directory runtime/projection regression failed after production analyzer PASS.'
}
...
if ($trackerHashAfter -ne $trackerHashBefore) {
  throw 'STOP: Seed-safe Directory repair modified the Dispatch tracker.'
}

Write-Host ''
Write-Host '============================================================' -ForegroundColor Green
Write-Host 'PIPE BUYER DIRECTORY SEED-SAFE REPOSITORY REPAIR PASSED' -ForegroundColor Green
Write-Host '============================================================' -ForegroundColor Green
...
Write-Host 'Candidate seeded-widget runtime without Firebase: PASS' -ForegroundColor Green
...
Write-Host 'Live mode lazily creates repository: PASS' -ForegroundColor Green
