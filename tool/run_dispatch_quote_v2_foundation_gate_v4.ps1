...
Write-Step 'Synchronizing the Quote V2 V4 exact-candidate support bundle'
& git fetch origin design/formal-beautification-foundation
if ($LASTEXITCODE -ne 0) {
  throw 'STOP: Could not fetch the Quote V2 V4 support bundle.'
}
$supportFiles = @(
...
if ($LASTEXITCODE -ne 0) {
  throw 'STOP: Could not synchronize the Quote V2 V4 support files.'
}
& git reset -q HEAD -- $supportFiles
if ($LASTEXITCODE -ne 0) {
  throw 'STOP: Could not unstage the Quote V2 V4 support files.'
}

Write-Step 'Parsing Quote V2 V4 controls before production mutation'
$nodeControls = @(
...
if ($parseErrors.Count -gt 0) {
  throw "STOP: Quote V2 V4 PowerShell parse failed: $($parseErrors[0].Message)"
}
Write-Host 'Quote V2 V4 control parse: PASS' -ForegroundColor Green
...
try {
  ...
  Write-Step 'Running analyzer-driven candidate hygiene across Dashboard and Jobs'
  & node '.\tool\clean_dispatch_quote_v2_candidate_hygiene.mjs'
  if ($LASTEXITCODE -ne 0) {
    throw 'STOP: Quote V2 analyzer-driven candidate hygiene failed. Production source was not changed.'
  }

  Write-Step 'Formatting and strictly analyzing the complete cleaned candidate graph'
  $candidateDart = @(
    '.\lib\marketplace\pipebuyer_dispatch_repository_quote_v2_preflight.dart',
    '.\lib\marketplace\pipebuyer_dispatch_dashboard_quote_v2_preflight.dart',
    '.\lib\marketplace\pipebuyer_dispatch_page_quote_v2_preflight.dart'
  ...
  if ($LASTEXITCODE -ne 0) {
    throw 'STOP: Quote V2 cleaned candidate Dart formatter failed. Production source was not changed.'
  }
  ...
  foreach ($target in $candidateDart) {
    ...
    if ($LASTEXITCODE -ne 0) {
      throw "STOP: Cleaned exact Quote V2 candidate still does not compile cleanly: $target"
    }
  }
  Write-Host 'Exact cleaned candidate repository/dashboard/page analyzer before mutation: PASS' -ForegroundColor Green
  ...
  $backupRoot = Join-Path $repoRoot "_local_backups\dispatch-quote-v2-v4-$stamp"
  ...
  Write-Step 'Promoting the exact analyzed candidate'
  ...
  if ($trackerHashAfter -ne $trackerHashBefore) {
    throw 'STOP: Quote V2 V4 gate modified the Dispatch tracker.'
  }
}
catch {
  $failure = $_
...
Write-Host 'PIPE BUYER DISPATCH QUOTE V2 V4 EXACT-CANDIDATE GATE PASSED' -ForegroundColor Green
Write-Host '============================================================' -ForegroundColor Green
...
Write-Host 'Analyzer-driven Dashboard + Jobs candidate hygiene: PASS' -ForegroundColor Green
Write-Host 'Exact cleaned candidate repository/dashboard/page analyzer: PASS' -ForegroundColor Green
