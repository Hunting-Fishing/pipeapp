...
Write-Host 'Timed Buying trust + participant experience: v4-syntax-safe-rollback-20260816' -ForegroundColor Green
Write-Host 'Recorded fix #1: the old v2 helper embedded Dart interpolation inside a JavaScript template literal.' -ForegroundColor DarkGray
Write-Host 'Recorded fix #2: rollback now restores only files whose SHA256 actually changed, with retry-safe byte writes.' -ForegroundColor DarkGray
Write-Host 'Do not use the older v1/v3 PowerShell runners after this version.' -ForegroundColor DarkGray
...
Step 'Current local work before syntax-safe trust pass'
git status --short

$backupRoot = Join-Path $env:TEMP "pipebuyer-trust-v4-$([guid]::NewGuid().ToString('N'))"
...
$complete = $false
try {
  Step 'Syntax-checking only parser-safe migration entrypoints'
  # v2 is legacy migration SOURCE and is intentionally read as text by v4.
  # Never node --check v2 directly; v4 sanitizes its unreachable legacy Dart
  # template before v3 imports it.
  foreach ($script in @($patchV3,$patchV4)) {
    & node --check $script
    if ($LASTEXITCODE -ne 0) { throw "Node syntax check failed: $script" }
  }
  Write-Host 'Parser-safe migration entrypoints passed.' -ForegroundColor Green

  Step 'Applying Timed Buying viewer trust and participation enhancements'
  & node $patchV4
  if ($LASTEXITCODE -ne 0) { throw 'Syntax-safe Timed Buying migration failed.' }
  ...
  & flutter test $trustTest --dart-define=PIPE_ENV=formal-beautification-local --dart-define=PIPE_RELEASE_SHA=timed-buying-trust-v4
  if ($LASTEXITCODE -ne 0) { throw 'Trust widget test failed.' }

  Step 'Running urgency and viewer-position regression contract'
  & flutter test $engagementTest --dart-define=PIPE_ENV=formal-beautification-local --dart-define=PIPE_RELEASE_SHA=timed-buying-trust-v4
  if ($LASTEXITCODE -ne 0) { throw 'Engagement widget test failed.' }

  Step 'Running Timed Buying action regression contract'
  & flutter test $presentationTest --dart-define=PIPE_ENV=formal-beautification-local --dart-define=PIPE_RELEASE_SHA=timed-buying-trust-v4
  if ($LASTEXITCODE -ne 0) { throw 'Presentation widget test failed.' }

  Step 'Rechecking compact professional Asset Overview'
  & flutter test $compactTest --dart-define=PIPE_ENV=formal-beautification-local --dart-define=PIPE_RELEASE_SHA=timed-buying-trust-v4
  ...
  Step 'TIMED BUYING TRUST V4 PASSED'
  ...
  Write-Host 'Known syntax and rollback failure modes are recorded in this runner.' -ForegroundColor Green
  git diff --stat
  git status --short
  $complete = $true
}
  ...
  if (-not $complete) {
    Write-Host "`nTrust v4 failed; checking which protected files actually changed before rollback." -ForegroundColor Red
    $rollbackOkay = $true
    ...
    if ($rollbackOkay) {
      Write-Host 'Protected product/backend files are back at their exact pre-run hashes.' -ForegroundColor Yellow
    }
    else {
