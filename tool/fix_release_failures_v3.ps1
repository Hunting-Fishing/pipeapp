...
if ($unexpected.Count -gt 0) {
  ...
  throw 'Only the V2 release-repair files may be dirty before running V3.'
}

$backupRoot = "${workspace}_release_fix_v3_backup_$(Get-Date -Format yyyyMMdd_HHmmss)"
...
Write-Host '==> Applying the final two test-harness repairs' -ForegroundColor Cyan
node .\tool\fix_release_failures_v3.mjs
Assert-NativeSuccess 'Final release test patcher'

dart format test/marketplace_grid_density_test.dart test/marketplace_offer_schedule_test.dart
...
Write-Host '==> Re-running the repaired release test groups' -ForegroundColor Cyan
flutter test @focusedTests --reporter expanded --concurrency=1 @defines
Assert-NativeSuccess 'Focused release tests'

Write-Host '==> Running the complete Flutter suite' -ForegroundColor Cyan
...
Write-Host '==> Staging the validated V2 + V3 release repairs' -ForegroundColor Cyan
git add -- @targets
...
Write-Host ''
Write-Host 'All Flutter release tests passed and the repairs were pushed to main.' -ForegroundColor Green
