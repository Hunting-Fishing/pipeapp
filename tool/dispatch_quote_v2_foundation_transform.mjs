...
function normalizeRepository(input) {
  ...
  const updateBefore = `  Future<void> updateBid({\n    required String bidId,\n    required num amount,\n    required String note,\n    required DateTime availableDate,\n    required String vehicleId,\n    required String vehicleName,\n  }) async {`;
  const updateAfter = `  Future<void> updateBid({\n    required String bidId,\n    required num amount,\n    required String note,\n    required DateTime availableDate,\n    required String vehicleId,\n    required String vehicleName,\n    required Map<String, dynamic> quoteBreakdown,\n    required String currency,\n  }) async {`;
  source = replaceOnce(source, updateBefore, updateAfter, 'Repository updateBid signature');
  ...
  const occurrences = source.split(payloadAnchor).length - 1;
  if (occurrences === 0) {
    if (!source.includes("'quoteBreakdown': quoteBreakdown")) {
      fail('Repository quote payload anchor was not found.');
    }
  } else {
    source = source.split(payloadAnchor).join(payloadReplacement);
  }
  if ((source.match(/'quoteBreakdown': quoteBreakdown/g) || []).length < 2) {
    fail('Repository bid and updateBid do not both transmit quoteBreakdown.');
  }
  return source;
}
...
function normalizePolicy(input) {
  ...
  const amountAnchor = `  const amount = requireMoney(data.amount, \"Carrier quote\");\n  const note = String(data.note || \"\").trim();`;
  const amountReplacement = `  const amount = requireMoney(data.amount, \"Carrier quote\");\n  const breakdown = validateDispatchQuoteBreakdown(data, amount);\n  const note = String(data.note || \"\").trim();`;
  source = replaceOnce(source, amountAnchor, amountReplacement, 'Dispatch quote policy calculation');
  source = replaceOnce(
    source,
...
function normalizeCommands(input) {
  ...
  const resultAnchor = `        revision,\n        created: !existingBid,\n      };`;
  const resultReplacement = `        revision,\n        quoteReference: values.quoteReference,\n        created: !existingBid,\n      };`;
  source = replaceOnce(source, resultAnchor, resultReplacement, 'Dispatch quote server result');
  return source;
}
...
export function transformDispatchQuoteV2Foundation(files) {
  ...
  for (const key of Object.keys(transformed)) {
    if (second[key] !== transformed[key]) fail(`Quote v2 transform is not idempotent for ${key}.`);
  }
  return transformed;
}
