...
function requiredReplace(before, after, label) {
  ...
  if (!source.includes(before)) {
    throw new Error(`Timed Buying attention migration could not find: ${label}`);
  }
...
// Use the stronger attention frame and the expanded time legend. Final-day
// listings now move; final-hour listings receive the critical dual-border glow.
...
// Legacy fixture/listing titles can still contain Auction even after the public
// UI migration. Normalize only display strings; stored transactionType remains
// Auction for compatibility with the established backend command model.
optionalReplace(
...
// Clean legacy seller descriptions such as "visual-sandbox auction" when they
// are rendered on a Timed Buying detail page.
optionalReplace(
...
// Detail-page participant position. This reuses the same bounded activity query
// already used by the offer history and never exposes another buyer's identity.
...
// Timed offer activity must show the viewer which rows are theirs and whether
// those offers are currently leading or have been surpassed.
...
// Update deterministic local fixture language. The 5-hour close remains useful
// for proving the final-day moving border without waiting for production time.
