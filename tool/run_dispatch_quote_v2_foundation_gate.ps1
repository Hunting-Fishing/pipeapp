...
Write-Step 'Synchronizing only the Dispatch Quote V2 foundation support bundle'
& git fetch origin design/formal-beautification-foundation
if ($LASTEXITCODE -ne 0) {
  throw 'STOP: Could not fetch the Dispatch Quote V2 support bundle.'
}

$supportFiles = @(
  'lib/marketplace/marketplace_dispatch_quote_form.dart',
  'tool/dispatch_quote_v2_foundation_transform_v2.mjs',
  'tool/verify_dispatch_quote_v2_foundation_dryrun.mjs',
  'tool/prepare_dispatch_quote_v2_foundation_candidate.mjs',
  'tool/apply_dispatch_quote_v2_foundation.mjs',
...
if ($LASTEXITCODE -ne 0) {
  throw 'STOP: Could not synchronize the focused Dispatch Quote V2 support files.'
}
& git reset -q HEAD -- $supportFiles
if ($LASTEXITCODE -ne 0) {
  throw 'STOP: Could not unstage the focused Dispatch Quote V2 support files.'
}

Write-Step 'Parsing every Quote V2 control before production mutation'
$nodeControls = @(
  '.\tool\dispatch_quote_v2_foundation_transform_v2.mjs',
  '.\tool\verify_dispatch_quote_v2_foundation_dryrun.mjs',
  '.\tool\prepare_dispatch_quote_v2_foundation_candidate.mjs',
  '.\tool\apply_dispatch_quote_v2_foundation.mjs'
)
...
if ($parseErrors.Count -gt 0) {
  throw "STOP: Quote V2 PowerShell parse failed: $($parseErrors[0].Message)"
}
Write-Host 'Quote V2 control parse before mutation: PASS' -ForegroundColor Green
...
Write-Step 'Running the exact-local-source Quote V2 dry-run'
& node '.\tool\verify_dispatch_quote_v2_foundation_dryrun.mjs'
if ($LASTEXITCODE -ne 0) {
  throw 'STOP: Quote V2 exact-local dry-run failed. Production source was not changed.'
}
foreach ($relative in $productionFiles) {
  $currentHash =
      (Get-FileHash -Algorithm SHA256 -LiteralPath (Join-Path $repoRoot $relative)).Hash
...
try {
  Write-Step 'Building transformed Quote V2 candidate files without modifying production'
  & node '.\tool\prepare_dispatch_quote_v2_foundation_candidate.mjs'
  if ($LASTEXITCODE -ne 0) {
  ...
  Write-Step 'Analyzing the isolated Quote V2 repository and dashboard candidate graph'
  ...
  Write-Host 'Candidate reusable repository/dashboard analyzer: PASS' -ForegroundColor Green
...
$backupRoot = Join-Path $repoRoot "_local_backups\dispatch-quote-v2-gate-$stamp"
...
try {
  Write-Step 'Applying only the already-preflighted Quote V2 foundation transformation'
  $mutationStarted = $true
  & node '.\tool\apply_dispatch_quote_v2_foundation.mjs'
  if ($LASTEXITCODE -ne 0) {
    throw 'STOP: Quote V2 source transformation failed. The gate will restore its backup.'
  }
  ...
  Write-Step 'Running the Quote V2 Flutter integration contract'
  & flutter test '.\test\dispatch_quote_v2_foundation_contract_test.dart' `
    --reporter expanded
  if ($LASTEXITCODE -ne 0) {
    throw 'STOP: Quote V2 Flutter integration contract failed.'
  }
...
Write-Host 'PIPE BUYER DISPATCH QUOTE V2 FOUNDATION PASSED' -ForegroundColor Green
Write-Host '============================================================' -ForegroundColor Green
Write-Host 'Reusable detailed quote form analyzer before mutation: PASS' -ForegroundColor Green
Write-Host 'Exact local dry-run before mutation: PASS' -ForegroundColor Green
