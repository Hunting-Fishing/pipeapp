...
$targets = @(
  ...
  'firebase/functions/scripts/seed_visual_sandbox.js',
  ...
  'test/marketplace_weight_catalog_test.dart'
)
...
try {
  ...
  Write-Step 'Adding weight sandbox fixture and deterministic seed order'
  & node '.\tool\patch_weight_sandbox_listing_v1.mjs'
  if ($LASTEXITCODE -ne 0) { throw 'Sandbox weight listing patch failed.' }
  & node '.\tool\patch_weight_sandbox_order_v1.mjs'
  if ($LASTEXITCODE -ne 0) { throw 'Sandbox weight seed-order patch failed.' }
