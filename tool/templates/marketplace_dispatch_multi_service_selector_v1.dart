...
class _MarketplaceDispatchMultiServiceSelectorState
  ...
  void didUpdateWidget(
    ...
    if (!_sameStrings(oldWidget.allowedServiceCodes, widget.allowedServiceCodes)) {
      final allowedCodes = _allowedServices.map((service) => service.code).toSet();
      _rows = _rows
          .map((value) => value != null && allowedCodes.contains(value) ? value : null)
  ...
  List<String?> _initialRows(List<String> values) {
    ...
    for (final value in values) {
      if (allowedCodes.contains(value) && !unique.contains(value)) unique.add(value);
        ...
        for (var index = 0; index < _rows.length; index++) ...[
          LayoutBuilder(
            builder: (context, constraints) {
              ...
              final otherSelected = <String>{..._selectedCodes}..remove(current);
              final items = services
                  .where((service) =>
                      service.code == current || !otherSelected.contains(service.code))
                  .map(
