...
function replaceRangeOnce(startMarker, endMarker, replacement, label) {
  const start = source.indexOf(startMarker);
  if (start < 0 || source.indexOf(startMarker, start + 1) >= 0) {
    throw new Error(`${label}: start marker must occur exactly once`);
  }
  const end = source.indexOf(endMarker, start + startMarker.length);
  if (end < 0) {
    throw new Error(`${label}: end marker not found after start marker`);
  }
  source =
