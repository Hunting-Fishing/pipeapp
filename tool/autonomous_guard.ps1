...
if (-not (Test-Path -LiteralPath $riskPolicyPath)) {
    throw "Missing autonomous risk policy: $riskPolicyRelative"
}
...
if ($changedPaths.Count -eq 0) {
    throw "Autonomous guard was invoked with no workspace changes."
}
...
& git diff --check HEAD --
if ($LASTEXITCODE -ne 0) {
    throw "git diff --check found whitespace or conflict-marker errors."
}
...
foreach ($relative in $changedPaths) {
    ...
    if ($text -match '(?m)^(<<<<<<< |=======\s*$|>>>>>>> )') {
        throw "Conflict marker detected in changed file: $relative"
    }
    foreach ($secretPattern in @($riskPolicy.secret_content_patterns)) {
        if ($text -match [string]$secretPattern) {
            throw "Potential credential/secret content detected in changed file: $relative"
        }
    }
    ...
    if ($extension -eq ".md" -and $currentLines -gt $maxDocumentLines) {
        if ($null -eq $baselineLines) {
            throw "New documentation file $relative is $currentLines lines; documentation maximum is $maxDocumentLines. Split by responsibility."
        }
        if ($currentLines -gt [int]$baselineLines) {
            throw "Legacy oversized documentation $relative grew ($baselineLines -> $currentLines). Split or reduce it rather than adding more content."
        }
    ...
    if ($currentLines -ge [int]$baselineLines) {
        throw "Oversized source file $relative grew or did not shrink ($baselineLines -> $currentLines lines). Files above $maxSourceLines must be reduced before adding responsibility."
    }
...
if (-not [string]::IsNullOrWhiteSpace($ResultPath)) {
    if (-not [System.IO.Path]::IsPathRooted($ResultPath)) {
        $ResultPath = Join-Path $ProjectRoot $ResultPath
    }
    if (-not (Test-Path -LiteralPath $ResultPath -PathType Leaf)) {
        throw "Structured worker result not found for risk validation: $ResultPath"
    }
    $result = Get-Content -LiteralPath $ResultPath -Raw | ConvertFrom-Json
    $declaredRisk = [string]$result.risk_level
    $declaredRank = Get-RiskRank -Risk $declaredRisk

    if ($declaredRank -lt $detected.Rank) {
        throw "Worker under-classified risk: declared $declaredRisk, path policy requires at least $($detected.Risk)."
    }
    if ($declaredRisk -eq "critical" -and -not [bool]$riskPolicy.critical_changes_allowed) {
        throw "Critical-risk changes are human-only and cannot be autonomously committed."
    }
    if (@($result.knowledge_used).Count -eq 0) {
        throw "Worker result did not record knowledge used."
    }
    if ($declaredRank -ge 2 -and @($result.compatibility_checks).Count -eq 0) {
        throw "Medium/high-risk work must record compatibility checks."
    }
    if ($declaredRank -ge 3 -and [string]::IsNullOrWhiteSpace([string]$result.rollback_notes)) {
        throw "High-risk work must record rollback/recovery implications."
    }
    if ($dependencyDetected -and -not [bool]$result.dependency_change) {
        throw "Dependency manifest/lockfile changed but worker declared dependency_change=false."
    }
    if ($dataDetected -and -not [bool]$result.data_change) {
        throw "Data/schema-related path changed but worker declared data_change=false."
    }
    if ($securityDetected -and -not [bool]$result.security_change) {
        throw "Security/auth/rules-related path changed but worker declared security_change=false."
    }
    if ($billingDetected -and -not [bool]$result.billing_change) {
        throw "Billing/payment-related path changed but worker declared billing_change=false."
    }
    if ($providerDetected -and -not [bool]$result.provider_change) {
        throw "Provider-related path changed but worker declared provider_change=false."
    }
    if ($declaredRank -ge 3) {
        $passedFocused = @($result.verification | Where-Object { $_.result -eq "passed" }).Count
        if ($passedFocused -eq 0) {
            throw "High-risk work must include at least one passed focused verification before supervisor review."
        }
    }
}
