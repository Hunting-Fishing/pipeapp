...
foreach ($required in @($v2, $recovery, $fingerprint)) {
    if (-not (Test-Path -LiteralPath $required)) { throw "Autonomous Builder source is missing: $required" }
}
. $fingerprint
...
# Clean worktrees are a no-op. Provably interrupted autonomous changes are
# preserved in a Git stash; ambiguous/operator changes are never touched.
