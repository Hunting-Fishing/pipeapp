...
const dashboard = transformed.dashboard.replace(
...
if (dashboard.includes('class _DispatchQuoteDialog extends StatefulWidget')) {
  throw new Error('STOP: Candidate dashboard still contains the retired quote editor.');
}

const page = transformed.page
  .replace(
    "import 'marketplace_dispatch_repository.dart';",
    "import 'pipebuyer_dispatch_repository_quote_v2_preflight.dart';",
  )
...
console.log('PIPE BUYER DISPATCH QUOTE V2 CANDIDATE V2 BUILT');
for (const [key, value] of Object.entries(output)) {
  console.log(`${key}: ${path.relative(repoRoot, value)}`);
}
console.log('Retired dashboard quote editor present: NO');
