...
$verifierContract = Join-Path $script:PipeBuyerRepoRoot 'test\dispatch_credential_verifier_formatter_contract_test.dart'

foreach ($required in @($doctor, $updateReminder, $normalizeDart, $verify, $verifierContract)) {
...
Write-Host 'Verifier structural contracts are formatter-tolerant and tested before the full verifier runs.' -ForegroundColor DarkGray
Write-Host 'Verification after bounded normalizers is source-read-only.' -ForegroundColor DarkGray
...
Run-CheckedCommand 'Proving credential verifier survives Dart formatter line wrapping' { flutter test $verifierContract }
...
Write-Host 'Formatter-tolerant verifier contract: PASS' -ForegroundColor Green
