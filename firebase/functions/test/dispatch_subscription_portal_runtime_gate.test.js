...
function authenticatedRequest(data = {}) {
  return {auth: {uid: "user-1", token: {}}, data};
}
        ...
        collection(name) {
          ...
          return {
            doc(id) {
              ...
              return {
                async get() {
                  return {exists: portal != null, data: () => portal};
                },
              };
            },
          };
...
test("runtime Portal decision requires complete stored proof bound to exact bpc", () => {
  const ready = dispatchBillingPortalRuntimeDecision(verifiedPortal());
  assert.equal(ready.ready, true);
  assert.equal(ready.configurationId, "bpc_live_dispatch");
  assert.equal(dispatchBillingPortalRuntimeDecision(verifiedPortal({providerVerified: false})).ready, false);
  assert.equal(dispatchBillingPortalRuntimeDecision(verifiedPortal({providerVerificationRevision: "old-policy"})).ready, false);
  assert.equal(dispatchBillingPortalRuntimeDecision(verifiedPortal({
    providerVerifiedFeatures: storedProviderFeatures({subscriptionUpdateTrialBehavior: "end_trial"}),
  })).ready, false);
  assert.equal(dispatchBillingPortalRuntimeDecision(verifiedPortal({providerVerifiedFeatures: {}})).ready, false);
});
...
test("unauthenticated request cannot trigger Firestore or Stripe provider reads", async () => {
  ...
  const gate = createDispatchSubscriptionPortalRuntimeGate(
      ...
      async () => { counters.invoked += 1; return {ok: true}; },
      {
        secretProvider: () => "sk_live_stub",
        stripeRequest: async () => { counters.providerCalls += 1; return liveProviderPortal(); },
      },
  );
  await assert.rejects(() => gate({data: {plan: "monthly"}}), /sign in before using Dispatch billing/i);
  assert.deepEqual(counters, {firestoreReads: 0, providerCalls: 0, invoked: 0});
});
...
test("production gate re-reads exact live bpc before delegating", async () => {
  ...
  const gate = createDispatchSubscriptionPortalRuntimeGate(
      ...
      async (request) => { invoked += 1; return {plan: request.data.plan}; },
      {
        secretProvider: () => "sk_live_stub",
        stripeRequest: async (call) => { providerCalls.push(call); return liveProviderPortal(); },
      },
  );
...
test("provider-side Portal drift that ends trials blocks protected billing actions", async () => {
  let invoked = 0;
  const gate = createDispatchSubscriptionPortalRuntimeGate(
      fakeAdmin(verifiedPortal()),
        ...
        stripeRequest: async () => liveProviderPortal({
          features: {
            ...
            subscription_update: {
              ...
              schedule_at_period_end: {conditions: []},
              products: [{product: DISPATCH_PORTAL_PRODUCT_ID, prices: [...DISPATCH_PORTAL_PRICE_IDS]}],
            },
          },
        }),
      },
