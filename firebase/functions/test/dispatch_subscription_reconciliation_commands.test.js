...
function providerInvoice({zero = false, wrongBillingType = false} = {}) {
  return {
    ...
    currency: "cad",
    charge: zero ? null : "ch_dispatch_1",
    parent: {
...
function providerFixture({zero = false, wrongBillingType = false} = {}) {
  const calls = [];
...
test("paid Dispatch invoice re-reads Invoice Charge and Balance Transaction and records balanced evidence", async () => {
...
test("zero-dollar promotional invoice reconciles from Invoice alone", async () => {
  const {admin, writes} = fakeAdmin(storedInvoice({
  ...
  assert.deepEqual(provider.calls, ["/v1/invoices/in_dispatch_1"]);
  assert.equal(writes.length, 3);
});
