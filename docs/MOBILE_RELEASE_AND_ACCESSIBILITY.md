...
Unauthenticated, refresh-safe `/privacy`, `/terms`, `/support`, and
`/account-deletion` routes now provide the public destinations required by the
mobile stores. Privacy and Terms expose only server-published reviewed policy
metadata and its exact HTTPS document; they fail closed rather than presenting
placeholder legal text. Controlled releases require a monitored
`PIPE_PUBLIC_SUPPORT_EMAIL`, while development builds clearly disclose when it
is absent. Proposed listing copy, screenshot coverage, and the external review
boundary are recorded in `docs/STORE_RELEASE_METADATA.md`. Hosted execution,
approved policy content, mailbox ownership, screenshots, and store-console
review remain uncredited until retained as release evidence.
