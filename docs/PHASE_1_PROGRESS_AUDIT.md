...
Branch: `agent/phase1-deep-links`
...
Overall Phase 1 launch readiness estimate: **71%**.
...
| Shareable routes | Source verified; staging/native association pending | Encoded GoRouter paths now restore listings, Auctions, public profiles, and participant-only conversations with loading, offline/access, unavailable, and return-home states. Cards, seller links, message lists, and simple conversation entry points use the routes; listing share copies a browser URL on web and a safe relative app path on native. Firebase Hosting already rewrites browser refreshes to `index.html`. Hosted refresh acceptance and approved-domain Android App Links/Apple Universal Links remain pending. |
