...
| 3 — Identity and abuse protection | No | 72% | Signup and sign-in enter a cross-platform ownership screen until Firebase Auth email and mobile-phone providers are verified. Protected Marketplace, Offer, Auction, Dispatch, messaging, reporting, and media commands require verified Auth claims; phone uniqueness is synchronized into a hashed server-owned registry. Separate transactional hourly quotas deduplicate identical retries and fail safely. Chat/report uploads require short-lived purpose, owner, target, path, size, and MIME grants. Administrator authorization now requires an audited custom role plus an MFA claim across Functions, Firestore, Storage, and Flutter; personal-email and public-profile bypasses are removed. Staging Phone Auth/App Check/MFA activation, Auth signup/listing-event controls, recovery, deletion, and export remain incomplete. |
...
Overall Phase 1 launch readiness estimate: **61%**.
