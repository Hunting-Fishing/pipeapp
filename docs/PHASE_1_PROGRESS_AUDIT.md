...
| --- | --- | ---: | --- |
| 0 — Scope lock and safe defaults | No | 85% | Runtime flags and high-risk defaults exist in source; the clean CI run and controlled Firebase deployment still need completion. |
| 1 — Environments, builds, diagnostics | No | 15% | CI and structured diagnostics foundations exist; Firebase environments, environment-selected configuration, deployment/rollback workflows, and clean artifact storage remain incomplete. |
